...
|------|--------|----------|
| Codex acceptance | pending | Drafted by Codex after Maintainer requested a detection-only repo audit |
...
2. PRD promoted list derives from `working-session/docs/README.md` or directly from the registry.
...
4. Any GuideCheck level claim must be verified from the relevant guide manifest and external anchor status before being advertised.
5. Agent-facing guidance must not imply Tokenese is adopted as a default.
...
1. PRD promoted count in `README.md`, `docs/index.html`, `docs/llms.txt`, and `assistant-guide.txt` matches the registry.
...
1. Overfitting the validator to exact prose.
   Mitigation: validate structured claim markers or constrained regexes rather than entire paragraphs.
