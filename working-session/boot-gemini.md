...
- `skills/` — Per-agent protocol skill files and shared metaskills:
  ...
  - `skills/gemini-3/SKILL.md` — Gemini's protocol execution guide (v0.1.0)
...
- **Skill file (PRD-012):** Gemini's protocol execution guide at `skills/gemini-3/SKILL.md` (v0.1.0). Explicit maintainer invocation only. Mandatory confirmation for all writes.
...
## Gemini-specific environment notes

### Sandboxed mode

Gemini CLI defaults to `sandboxed` mode for file writes. This means:
- File writes require plan-approval workflow (Gemini proposes changes, user approves).
- This aligns well with Turnfile's propose-only default (PRD-012 R5.1).
- When Maintainer grants apply intent, Gemini may need explicit user confirmation in the CLI before writes execute.

### Tool access

Gemini CLI provides:
- File read/write tools (subject to sandboxed mode)
- Shell command execution
- MCP client support for external tool integration
- 1M token context window

### Instruction loading

Gemini loads instructions from `GEMINI.md` files hierarchically:
- `~/.gemini/GEMINI.md` (user-global)
- Project root `GEMINI.md` (project-level — loads skill bundle via `@import`)
- Subdirectory `GEMINI.md` files (scoped overrides)

The project root `GEMINI.md` uses `@skills/gemini-3/SKILL.md` import to load the protocol skill.
