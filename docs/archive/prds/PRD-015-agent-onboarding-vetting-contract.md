...
5. **Skills structure scenario:** load and maintain required skills/instruction artifacts for the candidate runtime.
6. Scenario definitions and evidence templates are maintained at:
   - `working-session/docs/onboarding/ONBOARDING_TEST_SUITE.md`
   - `working-session/docs/onboarding/ONBOARDING_EVIDENCE_TEMPLATE.md`
7. Scenario language must remain candidate-agnostic; candidate identifiers appear only in run evidence.
8. Skills artifact conformance (R3.5) must be validated explicitly in the suite via a dedicated scenario.
...
8. `working-session/docs/onboarding/ONBOARDING_TEST_SUITE.md` (pilot scenario matrix and pass/fail criteria).

## Milestones

1. M0: Draft PRD-015.
2. M1: Define first onboarding scenario fixtures (`working-session/docs/onboarding/` staging).
...
2. ~~**OQ-048:** Should onboarding test fixtures live under `working-session/` only during pilot or move immediately to canonical `docs/` test assets?~~ **Resolved (updated 2026-02-11):** stage in `working-session/docs/onboarding/` during pilot; promote to top-level `onboarding/` after at least one completed onboarding run.

Tracked in: `working-session/OPEN_QUESTIONS.md`.

## Exit criteria for moving beyond working-session draft
