...
Version: 0.2.0 (pilot — aligned with Codex skill structure)
Protocol revision baseline: PRD-003/004/008/009 (finalized) + PRD-010/011/012/013 (hardened drafts)
...
4. Keep all substantive actions audit-visible in `inception/MAILBOX.md`, `inception/WORKLOG.md`, or `inception/TURNFILE.yaml`. (PRD-012 R5.2)
...
1. Read `inception/MAILBOX.md` inbox snapshot first on every active turn.
2. Re-check `inception/MAILBOX.md` before declaring turn complete.
...
1. Read `inception/boot-claude.md` — orientation, directory layout, current state.
2. Read `inception/TURNFILE.yaml` — coordination state: phase, tasks, locks, agent status, signals.
3. Read `inception/WORKLOG.md` status block (lines 1–11).
4. Read `inception/MAILBOX.md` inbox snapshot + unread messages addressed to Claude.
5. Read `inception/OPEN_QUESTIONS.md` — Active + Deferred sections only.
6. Read `inception/chat-claude.md` — session close snapshot from predecessor (bottom of file).
...
1. `inception/WORKLOG.md` status block — update "Now Working" and "Next Review Checkpoint" lines.
2. `inception/TURNFILE.yaml` — update task metadata (`status`, `completed_rev`, `notes`) and post coordination signal.
3. `inception/MAILBOX.md` — update message lifecycle status, inbox snapshot, and open queue when a thread changed.
4. After mailbox edits, regenerate `inception/MAILBOX.json`.
...
1. Read `inception/MAILBOX.md` (re-read immediately before any edit).
...
6. Regenerate MAILBOX.json: `node tools/export-mailbox-json.mjs inception/MAILBOX.md inception/MAILBOX.json`
...
4. **Update TURNFILE.yaml:**
   ...
   - Increment `coordination.revision`.
5. **Write session state snapshot** to bottom of `inception/chat-claude.md`:
   - YAML front matter: session ID, agent, timestamp, close reason, revision token (`REV-YYYYMMDD-snapshot-claude-<seq>-h<8hex>`).
   ...
   - Decision context.
   - OQs touched.
6. **Archive boot file** to `inception/archive/boot-claude/boot-claude_v<N>.md` (globally monotonic versioning per PRD-014).
7. **Write new boot file** with updated current state.
8. **Promote completed artifacts** from `inception/` to tracked `docs/` (copy, not move) — **only when explicitly directed by maintainer** and only for PRDs that pass the promotion gate (`PRD_STATUS.json` eligible = true + `tools/validate-prd-promotion.mjs` passes + maintainer acceptance evidence per PRD-006 R2a/R3).
...
1. Read `inception/OPEN_QUESTIONS.md`.
...
| Field | Value |
|-------|-------|
| Skill file version | 0.2.0 |
| Protocol baseline | PRD-003 R1, PRD-004 R1, PRD-008 R1-R4, PRD-009 R1-R2, PRD-010 R2-R6, PRD-011 R1-R3, PRD-013 R1-R7 |
| Policy test suite | PRD-012-M3-policy-test-suite.md (19 assertions, 4 scenario harnesses) |
| Last validated | M4 validation complete — all 4 scenarios PASS (rev 41) |
| Structural alignment | v0.2.0 aligned with Codex SKILL.md structure: front matter, execution contract, boundary discipline, startup read order, fallback rules, output format |
