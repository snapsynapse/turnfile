...
4. Keep all substantive actions audit-visible in `inception/MAILBOX.md`, `inception/WORKLOG.md`, or `inception/TURNFILE.yaml`.

## Active-Turn Boundary Discipline

1. Read `inception/MAILBOX.md` first for active-turn boundary checks.
2. Re-check `inception/MAILBOX.md` before declaring completion.
...
1. Read `inception/TURNFILE.yaml`.
2. Read `inception/WORKLOG.md` status block.
3. Read `inception/MAILBOX.md` inbox snapshot + assigned unread cards.
4. Read scope-specific protocol docs and PRDs.
5. Read `inception/OPEN_QUESTIONS.md` when work affects unresolved or deferred items.

## State Freshness Hooks

1. After milestone/task completion, refresh coordination artifacts in the same turn:
   - `inception/WORKLOG.md` status block
   - `inception/TURNFILE.yaml` task metadata / signal
   - `inception/MAILBOX.md` lifecycle status when a thread changed
2. After mailbox edits, regenerate `inception/MAILBOX.json`.
...
1. Updated message status and Ack/Reply lines in `inception/MAILBOX.md`.
2. Updated inbox/open-queue snapshot.
3. Regenerated `inception/MAILBOX.json` when mailbox changed.
...
Deterministic outputs:
1. Updated `inception/OPEN_QUESTIONS.md` lifecycle state (`active`, `deferred`, `resolved`).
